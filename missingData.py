...
if __name__ == "__main__":
    ...
    for ratio in missing_ratio:
        ...
        # test_index = [datetime.strptime((str(d)).replace('T', ' ').split('.')[0], '%Y-%m-%d %H:%M:%S').date() for d in list(data_test.index.values)]
        # fill_index = [datetime.strptime((str(d)).replace('T', ' ').split('.')[0], '%Y-%m-%d %H:%M:%S').date() for d in list(data_fill_plt.index.values)]
        # plt.plot(data_test.index, data_test['y'].values, "black", linestyle='-', label='真实值')
        # plt.plot(data_fill_plt.index, data_fill_plt['y'].values, "black", linestyle='--', label='填补值')
        ...
        # plt.autoscale(enable=True, axis='x', tight=True)  # 去掉坐标边缘的留白
        # plt.autoscale(enable=True, axis='y', tight=True)  # 去掉坐标边缘的留白
        # plt.plot(np.array(k_list).astype(dtype=np.str), rmse_list, 'black', linewidth=4)
        # plt.xlabel("K值", size=80)
        # plt.ylabel('${E_{RMSE}}$/A', size=80)
        ...
        # # plt.show()
        # # fig = plt.gcf()
        # plt.savefig(r'D:\SJTU\机器学习\小论文\数据补全\图\自动化设备-图\缺失率='+str(ratio)+'_RMSE.png',
        #             format='png',
        #             bbox_inches='tight',
        #             transparent=True)
        # plt.show()
